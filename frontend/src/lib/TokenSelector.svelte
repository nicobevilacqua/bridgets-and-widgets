<script lang="ts">
	import type { TokenSymbol, Token } from '../utils/types';
	
	export let tokens: Record<TokenSymbol, Token>;
	...
	function buttonClick(token: Token) {
		console.log(token);
		parentClick(token);
		closeParent();
	}
						...
						{#each Object.values(tokens) as token}
							<button
								...
								class="
									...
									bg-opacity-50
								"
								class:bg-[#5170ad]={token.symbol === 'ETH'}
							>
								<img src={token.icon} alt={token.name} class="w-10 h-10 ml-2 mr-4 rounded-full" />
								<div class="flex-col text-left">
									<div class="text-lg font-medium text-white">{token.name}</div>
								</div>
							</button>
						{/each}
					{/if}
